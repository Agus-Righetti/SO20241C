...
void manejar_truncado_archivo(char* nombre_archivo, int nuevo_tamanio, int pid)
{
    ...
    {                                   // Puede que haya espacio suficiente pero que no este contiguo, en ese caso hay que hacer compactacion
        ...
        int bytes_a_agregar = nuevo_tamanio - tamanio_original; // Bytes adicionales para alcanzar el nuevo tamaño del archivo
        int resto = tamanio_original % config_io->block_size; // Bytes restantes del ultimo bloque
        ...
        int bloques_necesarios = cantidad_total_bloques_que_ocupa(bytes_a_agregar);
        ...
        {
            bloques_necesarios = cantidad_total_bloques_que_ocupa(nuevo_tamanio); 
            ...
            {
                // Primero tenemos que sacar los bloques ocupados y poner la info en un buffer
                // despues compacto y pongo lo que habia sacado actualizando el bitmap
                int bloques_que_ocupa_original = cantidad_total_bloques_que_ocupa(tamanio_original);
            ...
            {   
                // Hay bloques contiguos, tengo que ponerlo en esos bloques

                int bloques_del_tamanio_original = cantidad_total_bloques_que_ocupa(tamanio_original);
    ...
    else if(tamanio_original > nuevo_tamanio) // Solo tengo que truncar el archivo, marco el bitmap como libre. Tengo que achicar el archivo.
    { 
        int bloques_a_liberar = cantidad_total_bloques_que_ocupa(tamanio_original) - cantidad_total_bloques_que_ocupa(nuevo_tamanio);
        int bloque_final_original = bloque_inicial + cantidad_total_bloques_que_ocupa(tamanio_original) - 1;

        for(int i = 0; i < bloques_a_liberar; i++) 
        {
            bitarray_clean_bit(bitmap, bloque_final_original - i);
        }
...
{
    int contador_bloques_libres_consecutivos = 0;

    for (int i = bloque_inicial; i < config_io->block_count; i++) 
    {
        if (!bitarray_test_bit(bitmap, i)) // Si es != quiere decir q esta libre
        {   // Si el bloque i está libre (bit es 0)

            contador_bloques_libres_consecutivos++; // Incrementa el cont de bloques libres consecutivos
            
            if (contador_bloques_libres_consecutivos == bloques_necesarios) 
            {
        ...
        {
            contador_bloques_libres_consecutivos = 0; // Reinicia el cont si se encuentra un bloque ocupado
        }
    }
...
char* leer_bloques(int bloque_inicial, int numero_bloques) 
{
    ...
    char* buffer = malloc(numero_bloques * config_io->block_size);

    fread(buffer, config_io->block_size, numero_bloques, archivo);
    fclose(archivo);
...
{ 
    ...
    for (int i = 0; i < config_io->block_count; i++) 
    { 
        if(bitarray_test_bit(bitmap, i)) // Si hay un bloque ocupado
        {
            ...
            actualizar_metadata(metadata);
            bitarray_set_bit(bitmap, contador_ocupados); // Actualizamos el bitmap para marcar el nuevo bloque ocupado como libre.
            bitarray_clean_bit(bitmap, i); // Actualizamos el bitmap para marcar el viejo bloque como libre.
            contador_ocupados++;
...
{   
    ...
    while(meta_aux->bloque_inicial != nro_bloque)
    {
        ...
        meta_aux->nombre_archivo = queue_pop(cola_archivos_en_fs); // Buscar en el siguiente archivo en cola

        // Trabajamos con el nuevo archivo, el siguiente en cola
